////////////////////////////////////////////////////////////////////////////////
// File Name  :2021202045_final_ls.c                                          //
// Date       :2023/04/12                                                     //  
...
// 'spls_final' that performs the same function as the optioned(r,S,h) 'ls'   //
...
void option_l(char *arr,char*path,int check,char*prev_dir,int hflag);
...
void wild(char*prev_dir, char*path,char*pattern,int check);
int main(int argc, char **argv){       //get parameter from terminal
   ...
   if(argc == 1 || (argc==2 && lflag==0 && aflag==0)){   //no parameter or argc==2 but no lflag and aflag
            ...
            for(int i=0; i<dir_num; i++){
             ...
             printf("%s\n", arr[i]);      //print sorting file
                ...
                wild(prev_dir,path,pattern,1);  // wild function
                }
                }
            }
                for(int i=0; i<dir_num; i++){
           ...
           dirp = opendir("."); //automatically search currnet directory
            ...
            if(rflag == 1){  
             for(int i=dir_num-1; i>=0; i--){
                printf("%s\n", arr[i]);      //print sorting file in converse
             }
            }
            else{
            for(int i=0; i<dir_num; i++){
        ...
        return 0;
           ...
           if(rflag == 1){ //if rflag is 1
             for(int i=dir_num-1; i>=0; i--){
               option_l(arr[i],pathname,local_check,pathname,hflag); //go to option_l function
             }
            }
            else{
            for(int i=0; i<dir_num; i++){
                option_l(arr[i],pathname,local_check,pathname,hflag); //call option_l function 
             }
            }
            for(int k=0; k< dir_num; k++){   //deallocate arr
                 free(arr[k]);
  ...
  for(int it=optind;it<argc;it++){    
      ...
      printf("%s\n", arr_file[i]);      //print sorted file
        ...
        for(int i=0;i<directory_num;i++){  
           ...
           printf("cannot access \'%s\' : No such directory\n", arr_dir[i]);
            ...
            if(rflag == 1){    //if rflag is 1
             for(int ip=dir_num-1; ip>=0; ip--){  //print in converse
                printf("%s\n", arr[ip]);      //print sorting file
             }
            }
            else{
    ...
    else if(lflag ==1 && rflag==0){ //option l
        ...
        printf("Directory path: %s\n",pathname);
      option_l(arr_file[i],pathname,local_check,pathname,0); //call option_l function
       }
        ...
        for(int i=0;i<directory_num;i++){
           ...
           printf("total : %d\n",total/2);}
            ...
            for(int j=0; j<dir_num; j++){
                option_l(arr[j],path,local_check,prev_dir,hflag); //call option_l function 
            }
    ...
    //printf("origin : %s",origin);
        ...
        for(int i=0;i <pat_num; i++)
        {
            ...
            if(exist==1){
                ...
                printf("Directory path: %s\n",path);    //if not dir -> print
                wild(prev_dir,path,pattern,1); //go to wild function
                }
                }
            }
                for(int i=0; i<f_num; i++){
                ...
                printf("%s\n",arr[i]);    //if not dir -> print
                wild(prev_dir,path,pattern,2); //go to wild function
                }
                }
            }
                for(int i=0; i<f_num; i++){
...
void sort_hidden(int dir_num, char*arr[],int max){
    ...
    for (int i =0; i<dir_num-1; i++){        //like bubble-sorting
      for(int j= i+1; j<dir_num; j++){
            char *big_arr1 = strdup(arr[i]);
            char *big_arr2 = strdup(arr[j]);
            for (int k = 0; k < strlen(big_arr1); k++) {
                big_arr1[k] = toupper(big_arr1[k]);
            }
            for (int k = 0; k < strlen(big_arr2); k++) {
                big_arr2[k] = toupper(big_arr2[k]);
            }
            if(arr[i][0]=='.' && arr[j][0] =='.'){
